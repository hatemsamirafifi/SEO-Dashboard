...
function extractCodeFromRecord(record: Record<string, unknown>): number | null {
  const errObj = isRecord(record.error) ? record.error : null;
  const code =
    record.status_code ?? record.statusCode ?? record.code ?? errObj?.code;
...
export function extractSafeDataforseoErrorMessage(
  ...
  // Application task error inside HTTP 200 with paused account or credit issues
...
function parseTransportFailure(
  ...
  if (
    /CONNECTION_FAILED|ECONNREFUSED|ECONNRESET|fetch failed/i.test(scrubbed)
  ) {
    return {
      provider: "DataForSEO",
      endpoint,
...
function parseKeywordFallbackError(
  ...
  if (isCreditHeuristic) {
    const is402Http = lower.includes("402");
    return {
      provider: "DataForSEO",
      endpoint,
      httpStatus: is402Http ? 402 : 200,
      dataforseoStatusCode: is402Http ? 40200 : 40200,
...
export function parseDataforseoDiagnosticsFromErrorMessage(
  ...
  if (httpMatch) {
    ...
    const endpoint = httpMatch[2]
      ...
      : defaultEndpoint;
    const isNoBody =
      httpMatch[6] === "no response body" ||
      scrubbed.includes("(no response body)");
    ...
    if (httpStatus >= 500) {
      ...
      parsedCode === 40201 ||
      msg.toLowerCase().includes("precaution") ||
