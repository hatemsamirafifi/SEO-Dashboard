...
// SDK-free home for the backlinks response schemas. The section module
// `backlinks.ts` statically imports dataforseo-client, so its exports must
// stay out of the worker's eager startup graph; feature code that needs these
// schemas (e.g. service layers routed through the DataRouter) imports them
// from here or from the SDK-free barrel — never from the section file.
//
// DataForSEO ships both the misspelled (`*_reffering_*`) and corrected keys;
// we accept both via passthrough so callers can read whichever is present.
...
export type DomainPageSummaryItem = z.infer<
  typeof domainPageSummaryItemSchema
>;
